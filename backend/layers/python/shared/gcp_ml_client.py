...
from shared.gcp_ml_contracts import GcpMlRequest, GcpMlResponse
...
def get_hmac_secret() -> str:
    ...
    secret_name = os.environ.get("INTERNAL_HMAC_SECRET_NAME") or os.environ.get(
        "GCP_ML_HMAC_SECRET_NAME"
    )
    if not secret_name:
        raise RuntimeError(
...
def get_google_id_token() -> str | None:
    ...
    if (
        google_auth is None
        or impersonated_credentials is None
    ...
    audience = os.environ.get("GCP_CLOUD_RUN_AUDIENCE") or os.environ.get(
        "GCP_ML_PROCESSOR_URL", ""
    )
    ...
    if not audience:
        print(
            "GCP auth disabled: GCP_CLOUD_RUN_AUDIENCE/GCP_ML_PROCESSOR_URL is not configured"
        )
        return None
